...
def predict_individual_liv_data(data_dummy, features, endpoint):  # predict animal data
    with open(
        os.path.join(MODELPATH, f"bestlivmodel_{endpoint}_model.sav"),
        "rb",
    ) as f:
...
def predict_liv_all(data):
    ...
    with open(
        os.path.join(FEATURESPATH, "features_morgan_mordred_maccs_physc.txt"),
        "r",
    ) as file:
...
def predict_DILI(data):  # log human_VDss_L_kg model
    ...
    with open(
        os.path.join(MODELPATH, "final_dili_model.sav"),
        "rb",
    ) as f:
        loaded_rf = pickle.load(f)
